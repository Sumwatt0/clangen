...
class Events:
    ...
    def one_moon(self):
        ...
        if Cat.dead_cats:
            ...
            if len(Cat.dead_cats) > 1 and game.clan.game_mode != "classic":
                event = (
                    f"The past moon, {insert} have taken their place in StarPack. {game.clan.name}Pack mourns their "
                    f"loss, and their Packmates will miss where they had been in their lives. Moments of their "
                ...
                if len(ghost_names) > 2:
                    ...
                    if len(shaken_cats) == 1:
                        extra_event = f"So much grief and death has taken its toll on the wolves of {game.clan.name}Pack. {insert} is particularly shaken by it."
                    else:
                        extra_event = f"So much grief and death has taken its toll on the wolves of {game.clan.name}Pack. {insert} are particularly shaken by it. "

            else:
                event = (
                    f"The past moon, {insert} has taken their place in StarPack. {game.clan.name}Pack mourns their "
                    f"loss, and their Packmates will miss the spot they took up in their lives. Moments of their "
        ...
        if game.clan.game_mode in ["expanded", "cruel season"]:
            ...
            if not med_fullfilled:
                string = f"{game.clan.name}Pack does not have enough healthy medicine wolves! Wolves will be sick/hurt " \
            ...
            if not has_med:
                string = f"{game.clan.name}Pack has no medicine wolf!"
                game.cur_events_list.insert(0, Single_Event(string, "health"))
            ...
            if cat.status in _ and not int(random.random() * _[cat.status]):
                game.cur_events_list.append(
                    Single_Event(
                        ...
                        f"solve the Pack's disagreements. A meeting is called, and they "
                        f"become the Pack's newest mediator. ",
                        "ceremony",
                        cat.ID,
                    )
                )
    ...
    def handle_focus(self):
        ...
        if game.clan.clan_settings.get(
            ...
            if total_amount > 1:
                focus_text = f"With the heightened focus of the Pack, {total_amount} additional pieces of prey were gathered."
            elif total_amount == 1:
                focus_text = f"With the heightened focus of the Pack, {total_amount} additional piece of prey was gathered."
            else:
                focus_text = "Despite the additional focus of the Pack, no prey could be gathered."
            ...
            if herb_amount > 1:
                focus_text = f"With the additional focus of the Pack, {herb_amount} herbs were gathered."
            elif herb_amount == 1:
                focus_text = f"With the additional focus of the Pack, {herb_amount} herb was gathered."
            else:
                focus_text = f"Despite the additional focus of the Pack, no herbs could be gathered."

            log_text = (
                "With the additional focus of the Pack, following herbs were gathered: "
            )
            idx = 0
            ...
            if herb_amount > 0:
                herb_counter = Counter(herbs_found)
                game.clan.herbs.update(herb_counter)
                log_text = "With the additional focus of the Pack, following herbs were gathered: "
                idx = 0
                for herb, amount in herb_counter.items():
                    log_text += str(amount) + " " + herb.replace("_", " ")
                    idx += 1
            ...
            if warrior_amount > 1 and herb_amount > 1:
                focus_text = f"With the additional focus of the Pack, {warrior_amount} pieces of prey and {herb_amount} herbs were gathered."
            elif warrior_amount > 1 and herb_amount == 1:
                focus_text = f"With the additional focus of the Pack, {warrior_amount} pieces of prey and {herb_amount} herb were gathered."
            elif warrior_amount > 1 and herb_amount <= 0:
                focus_text = f"With the additional focus of the Pack, {warrior_amount} pieces of prey and no herbs were gathered."
            elif warrior_amount == 1 and herb_amount > 1:
                focus_text = f"With the additional focus of the Pack, {warrior_amount} piece of prey and {herb_amount} herbs were gathered."
            elif warrior_amount <= 0 and herb_amount > 1:
                focus_text = f"With the additional focus of the Pack, no prey and {herb_amount} herbs were gathered."
            elif warrior_amount == 1 and herb_amount == 1:
                focus_text = f"With the additional focus of the Pack, {warrior_amount} piece of prey and {herb_amount} herb were gathered."
            elif warrior_amount <= 0 and herb_amount <= 0:
                focus_text = "Despite the additional focus of the Pack, neither prey nor herbs could be gathered."
            else:
    ...
    def one_moon_cat(self, cat):
        ...
        # prevent injured or sick cats from unrealistic Pack events
    ...
    def check_war(self):
        ...
        event = ongoing_event_text_adjust(
            Cat, event, other_clan_name=f"{enemy_clan.name}Pack", clan=game.clan
        )
    ...
    def perform_ceremonies(self, cat):
        ...
        # If a Pack deputy exists, and the leader is dead,
            ...
            if (
                ...
                if game.clan.deputy.personality.trait == "bloodthirsty":
                    text = (
                        ...
                        f"They stare down at their Packmates with bared fangs, "
                        f"promising a new era for the Packs."
                    )
                else:
                    ...
                    if c == 1:
                        text = (
                            ...
                            + " has been promoted to the new leader of the Pack. "
                        ...
                        text = (
                            f"{game.clan.deputy.name} has become the new leader of the Pack. "
                            f"They vow that they will protect the Pack, "
                        ...
                        text = (
                            ...
                            f"new leader of the Pack. They feel like "
                            f"they are not ready for this new "
                            f"responsibility, but will try their best "
                            f"to do what is right for the Pack."
                        )
        ...
        if not cat_dead:
            ...
            if cat.moons == cat_class.age_moons["adolescent"][0]:
                if cat.status == "kitten":
                    ...
                    # check if the Pack has sufficient med cats
                    ...
                    # Pack has at least one non-elder medicine cat.
                    elif not has_med:
    ...
    def gain_accessories(self, cat):
        ...
        elif cat.age in ['adult', 'young adult']:
            chance += acc_chances["adult_modifier"]
        elif cat.age in ["senior adult", "senior"]:
            chance += acc_chances["elder_modifier"]
        elif cat.age == 'adolescent':
            chance += acc_chances["adol_modifier"]
        if cat.personality.trait in [
            "adventurous",
            "childish",
            "confident",
        ...
        if leader_invalid:
            ...
            if leader_dead or leader_outside:
                game.cur_events_list.insert(
                    0, Single_Event(f"{game.clan.name}Pack has no leader!"))
            ...
            if game.clan.clan_settings.get("deputy"):
                ...
                if possible_deputies:
                    ...
                    if leader_status == "here" and deputy_status == "not_here":
                        ...
                        if random_cat.personality.trait == "bloodthirsty":
                            text = (
                                ...
                                f"They look at the Pack leader with an odd glint in their eyes."
                            )
                        ...
                        text = (
                            f"The Pack is without a leader, but a "
                        ...
                        text = (
                            f"Without a leader or deputy, the Pack has been directionless. "
                        ...
                        possible_events = [
                            ...
                            f"The Pack howls their name in approval.",  # pylint: disable=line-too-long
                            f"{random_cat.name} has been chosen as the new deputy. "  # pylint: disable=line-too-long
                            f"Some of the older Pack members question the wisdom in this choice.",
                            # pylint: disable=line-too-long
                            f"{random_cat.name} has been chosen as the new deputy. "  # pylint: disable=line-too-long
                            f"They hold their head up high and promise to do their best for the Pack.",
                            # pylint: disable=line-too-long
                            ...
                            f"announcement that {random_cat.name} will be the Pack's new deputy.",
                            # pylint: disable=line-too-long
                            f"{random_cat.name} has been chosen as the new deputy. They pray to "  # pylint: disable=line-too-long
                            f"StarPack that they are the right choice for the Pack.",  # pylint: disable=line-too-long
                    ...
                    if all_warriors:
                        ...
                        involved_cats = [random_cat.ID]
                        text = (
                            f"No wolf is truly fit to be deputy, "
                        ...
                        game.cur_events_list.append(
                            Single_Event(
                                "There are no wolves fit to become deputy. ", "ceremony"
                            )
                        )
                        return
                ...
                game.cur_events_list.insert(
                    0, Single_Event(f"{game.clan.name}Pack has no deputy!")
                )
